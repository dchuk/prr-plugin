...
- **Persisting results:** This command does not ship with a dedicated template. To archive the assessment, write it to `./artifacts/health-YYYY-MM-DD.md` as plain markdown with the scoring table, total, interpretation, and next steps. For the structured roadmap and theme files this assessment evaluates, see [`../skills/product-roadmaps/templates/`](../skills/product-roadmaps/templates/).
